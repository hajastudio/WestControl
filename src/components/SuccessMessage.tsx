...
export function SuccessMessage() {
  ...
  // Primeiro, adicione o estado para armazenar os dados do customer
  const [customerData, setCustomerData] = useState<CombinedData | null>(null);
  ...
  const formatAddress = () => {
    // Prioriza os dados do customer, caso contrário usa os dados do lead
    const addressData = customerData || leadData;
    ...
    if (addressData?.street) {
      const streetPart = `${addressData.street}${addressData.number ? `, ${addressData.number}` : ''}`;
    ...
    if (addressData?.city || addressData?.state) {
      const cityStatePart = `${addressData.city || ''}${addressData.state ? ` - ${addressData.state}` : ''}`;
      parts.push(cityStatePart);
    }
    if (addressData?.cep) {
      parts.push(`CEP: ${addressData.cep}`);
    }
